...
export function SearchBar({ storeName, departments, defaultQuery, defaultDept = '' }: SearchBarProps) {
  return (
    <form action="/s" method="get" role="search" className="flex h-[38px] min-w-0 flex-1 overflow-hidden rounded-[8px] bg-white focus-within:ring-[3px] focus-within:ring-[rgb(255_153_0_/_0.6)]">
      <label className="sr-only" htmlFor="dept">Category</label>
      <select id="dept" name="dept" className="hidden max-w-[130px] shrink-0 bg-surface-2 px-2 text-[12px] text-ink-2 outline-none sm:block" defaultValue={defaultDept}>
