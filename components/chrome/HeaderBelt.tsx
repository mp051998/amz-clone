...
export function HeaderBelt({ store, cartCount = 0, userName, deliverTo, langSlot }: HeaderBeltProps) {
  ...
  const search = <SearchBar storeName={store.name} departments={searchDepts} />;
  return (
    <div className="bg-nav-belt text-white">
      <div className="mx-auto max-w-[1500px] px-2 sm:px-3">
        <div className="flex h-[60px] items-center gap-1 sm:gap-2">
          <a href="/" aria-label={store.name} className="shrink-0 rounded-[3px] px-1 py-1 hover:outline hover:outline-1 hover:outline-white sm:px-2"><Wordmark tld={tld} /></a>
          ...
          <a href={userName ? '/account' : '/signin'} className="hidden shrink-0 rounded-[3px] px-2 py-1 leading-[14px] hover:outline hover:outline-1 hover:outline-white md:block">
            <span className="text-[12px]">{userName ? `Hello, ${userName}` : 'Hello, sign in'}</span>
            <div className="text-[14px] font-bold">Account &amp; Lists</div>
          </a>
          <a href="/orders" className="hidden shrink-0 rounded-[3px] px-2 py-1 leading-[14px] hover:outline hover:outline-1 hover:outline-white md:block">
            <span className="text-[12px]">Returns</span>
            <div className="text-[14px] font-bold">&amp; Orders</div>
          </a>
          <a href="/cart" aria-label={`Cart, ${cartCount} items`} className="flex shrink-0 items-end gap-1 rounded-[3px] px-1 py-1 hover:outline hover:outline-1 hover:outline-white sm:px-2">
