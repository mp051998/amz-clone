...
export function ProductRail({ title, seeMoreHref, products }: ProductRailProps) {
    ...
    <section className="bg-white p-5 shadow-[0_1px_2px_rgba(15,17,17,0.15)]">
      ...
      <div className="relative">
        <div ref={track} className="flex gap-4 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {products.map((p) => (
            <a key={p.id} href={`/product/${p.id}`} className="group w-[170px] shrink-0">
              ...
              <div className="mt-1">
                <Price minor={p.priceMinor} currency="USD" listMinor={p.listMinor} size={18} />
              </div>
